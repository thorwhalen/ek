...
- `run_suite(agent, tasks, *, k, check, price, run, seed)` — k trials/task, injected checker, per-slice, persisted.
- `agent_regression_gate` — compares **intervals, not points** (Wilson on the success rate, bootstrap on `pass^k` and on the skewed cost-per-success ratio). Fails only when the *whole* current interval sits on the wrong side. An overlapping interval is **not a pass** — it is an underpowered experiment, and the gate warns.
